...
system("mkdir output/sub-domains/#{domain} || echo Directory Created")
  ...
  system("subfinder -d #{domain} -o output/sub-domains/#{domain}/#{domain}_subfinder.txt")
  ...
  system("amass enum -d #{domain} -o output/sub-domains/#{domain}/#{domain}_amass.txt")
  puts "Amass Ended..."
  puts
  #Get it from https://github.com/christophetd/censys-subdomain-finder
  puts "+--------------------------------Subdomains BY Censys-------------------------------------+"
  system("python censys-subdomain-finder/censys_subdomain_finder.py #{domain} -o output/sub-domains/#{domain}/#{domain}_subfinder.txt")
  puts "Censys Ended..."
  puts
